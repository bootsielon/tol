...
Set tolphost=192.168.10.16

Set _url_bin_=http://%tolphost%/pub/bin/win
...
Rem Verifico que ejecutemos desde la consola de Visual Studio
If Not Defined VCINSTALLDIR (
  Set _msg_error=Solo se puede compilar desde una consola Visual Studio C++
  Goto ERROR
)

If Not Defined _windir (
  If Not Defined VS80COMNTOOLS (
    If Not Defined VS90COMNTOOLS (
      Set _msg_error=VS80COMNTOOLS o VS90COMNTOOLS no definida. Ejecute este bat desde una consola de Visual Studio C++.
      Rem Goto _VSDONE
    ) Else (
...
If Not Exist ..\..\%_mod%\%_windir%\%_sln% (
...
If Not Exist ..\..\%_mod%\%_windir%\postbuild.bat (
  xcopy /F /Y postbuild.bat ..\..\%_mod%\%_windir%
)
vcbuild %_rbopt% /useenv ..\..\%_mod%\%_windir%\%_sln% "Release|Win32"
If errorlevel 1 (
...
If "%_dodist%"=="1" (
  If Exist ..\NSIS\Tolbase*.exe (
    ...
    Echo copiando last_official_tol_release_win.txt hacia www.tol-project.org
    ..\ActiveTOL\bin\tol -v -c"Text WriteFile(\"last_official_tol_release_win.txt\",Version);"
    pscp last_official_tol_release_win.txt tdt@cvs.tol-project.org:/home/bayes/pub/bin/win/.
