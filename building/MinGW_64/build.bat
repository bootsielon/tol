...
if "%project%"=="tol" (
  set tol_contribs=%tolp_root%\toldevel-rtools-m64
  echo.  tol_contribs=!tol_contribs!
)
if exist "%tolp_trunk%\%project%\CodeLite64\%mode%" (
...
cd "%tolp_trunk%"
if "%is_built%"=="y" rmdir /S /Q "%tolp_trunk%\%project%\CodeLite64\%mode%"
mkdir "%tolp_trunk%\%project%\CodeLite64\%mode%"
:skip_mkdir

cd "%tolp_trunk%\%project%\CodeLite64\%mode%"

echo.
set /p cont="cmake? (y/n) "
if "%cont%"=="n" GOTO skip_cmake
if "%project%"=="tol" (
  cmake -DCLAPACK_NAME=lapacke -DTOL_EXTERNAL_DEVEL_DIR="%tol_contribs%" -DCMAKE_TOOLCHAIN_FILE="%tolp_trunk%\tol\cmake\mingw-m64.toolchain.cmake" -DCMAKE_BUILD_TYPE=%mode% -G"CodeLite - MinGW Makefiles" "%tolp_trunk%\%project%"
) else (
  cmake -DTOL_PREFIX_PATH="%tolp_trunk%\tol\CodeLite64\Release\Runtime_Base" -DCMAKE_TOOLCHAIN_FILE="%tolp_trunk%\tol\cmake\mingw-m64.toolchain.cmake" -DCMAKE_INSTALL_PREFIX= -DCMAKE_BUILD_TYPE=%mode% -G"CodeLite - MinGW Makefiles" "%tolp_trunk%\%project%"
)
:skip_cmake
