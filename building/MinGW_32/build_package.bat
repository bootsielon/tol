...
set init_dir=%CD%
set tolp_runtime=%tolp_root%\trunk\tol\CodeLite\Release\Runtime_Base
...
if exist "%package_dir%\CMakeLists.txt" (
  cd %package_dir%
  set cmake_dir=BUILD_CMAKE\m32\release
) else (
...
if "!answer!"=="y" (  
  cmake -DTOL_PREFIX_PATH=%tolp_runtime% -DCMAKE_TOOLCHAIN_FILE=%tol_cmake_dir%\mingw-m32.toolchain.cmake -DCMAKE_BUILD_TYPE=Release -G"CodeLite - MinGW Makefiles" ..\..\..
)
