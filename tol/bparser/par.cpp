...
//! initialize lastSymbol_ static member of BBParser to NIL value
BToken* BParser::lastSymbol_ = NIL;
 ...
 */
BParser::BParser(): openNumber_ (0), complete_ (BFALSE) {
    filter_  = new BFilter;
    scan_ = new BScanner;
}
			       ...
			       const BText& endLine)
{
	...
	if(tok && !txt.HasName()) 
  {
		    ...
		    if(aux.Get(0)=='\"')
        {
          ...
          BText aux2 = aux1.Replace("\"","\\\"");
          BText aux3 = aux2.Replace("\\","\\\\");
          aux = BText("\"")+aux3+"\"";
        }
		...
		case MONARY:
		{
		    ...
		    if((b = b->cdr())) {
			txt += BParser::Unparse(
			    (const List*) Tree::treLeft((List*)tre),
			    indent,endLine);
		    }
		...
		case FUNCTION:
		{
			...
			txt += open.Name();
			txt += BParser::Unparse(
			    (List*) (b->car()), indent, endLine);
			while((b = b->cdr())) {
			    txt += BText(", ") + BParser::Unparse(
				(List*) (b->car()),indent,endLine);
			}
		    }
		    txt += close;
...
Tree* BParser::ParseOpen (Tree* tre) 
{
  ...
  openNumber_++;
  if(complete_ || (arg && !filter_->IsFirstIdentifier(arg->String()[0]))) 
  {
...
Tree* BParser::ParseType (Tree* tre) {
    if(NextArgument() || complete_) {
        messageError_+=
            I2("Unexpected type symbol "+ NextSymbol()->Name(),
	       "Símbolo de tipo "+NextSymbol()->Name()+" fuera de lugar");
    } else {
...
Tree* BParser::ParseSymbol (Tree* tre, BCloseToken* close) {
    BTokenType firstType;
    do {
	BBool ok = BTRUE;
	BBool structFound = BFALSE;
	if(scan_->NextSymbol()) {
	    const BText& sym = scan_->NextSymbol()->Name();
	    structFound = (sym=="Struct");
	}
	firstType = scan_->ReadNextSymbol();
	if((firstType==ARGUMENT) ||
	   ((scan_->NextArgument()!="") && complete_)) {
	    BText arg = scan_->NextArgument();
	    filter_->UnReplace(arg);
	    messageError_+=
		I2("Unexpected argument "+ arg,
		...
		scan_->AddSymbol(new BTypeToken(arg->Name()));
...
BText BParser::ErrorPosition (BInt& row, BInt& col)
{
	...
	if(pos1 && ((i+1)<margin)) { // do not get another char from the code
	    pos1--; first--;         // because the next iteration of i is 
	}                            // the last
    }
    if(!pos1) {
	first = 1;
