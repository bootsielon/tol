...
//--------------------------------------------------------------------
void BModel::CalcResiduous(BArray<BDat>&res)
//--------------------------------------------------------------------
{
  BInt j;
  if(maxpq_==0)
...
//--------------------------------------------------------------------
{
  ...
  //Std(BText("\npiw_.Size()=")<<piw_.Size());
    ...
    {
      Warning(I2("Cannot calculate values for interruptions, probably due "
                 "to a numerical problem in SVD of partial PI-weights matrix.",
                 "No se pudieron calcular los valores de las interrupciones "
                 "debido probablemente a un error de tipo numérico en la "
                 "SVD de la matriz parcial de los PI-weights."));
      A_ = A*BDat::PosInf();
    }
  }
}
