...
//--------------------------------------------------------------------
// autotools adoption project 
// - next code is different from gnu_tollib ->
...
#define _ZERO_(X) (fabs(X)<=eps1)
#define _POS_(X)  ((X)>eps1)
#define _POSZ_(X)  ((X)>=-eps1)
#define _NEG_(X)  ((X)<-eps1)
#define _NEGZ_(X)  ((X)<=eps1)
#define _EQ_(X,Y) _ZERO_((X)-(Y))
#define _NE_(X,Y) _POS_(fabs(X-Y))
#define _GT_(X,Y) _POS_(X,Y)
#define _GE_(X,Y) _POSZ_(X-Y)
 ...
 */
/*
     Use lower bound instead that crazy value = 5.0e0
 */
        *s = 0.0e0;
        T5 = atol;
        T6 = tol;
...
S20:
    ...
    dmin = spmpar(&KMIN1);
    if (!(fsmall>spmpar(&KMIN1))) goto S30;
    *status = -1;
    *qleft = *qhi = 1;
    return;
S30:
...
S200:
    yy = *fx;
    /* qbdd = qincr && yy <= 0.0e0 || !qincr && yy >= 0.0e0; */
    qbdd = qincr && !(yy>spmpar(&KMIN1)) || !qincr && yy >= 0.0e0;
    qlim = xlb <= small;
    qcond = qbdd || qlim;
    if(qcond) goto S210;
    step = stpmul*step;
...
/*     if(!(*fx < 0.0e0)) goto S30; */
    ...
    if(_POSZ_(fb)) goto S40;
    if(_POSZ_(*fx)) goto S30;
    *status = -1;
    *qleft = *fx < fb;
    *qhi = 0;
    return;
...
/*     if(!(*fx > 0.0e0)) goto S50; */
    ...
    if(_NEGZ_(fb)) goto S60;
    if(_NEGZ_(*fx)) goto S50;
    *status = -1;
    *qleft = *fx > fb;
    *qhi = 1;
    return;
...
/*     if(!(fabs(fc) < fabs(fb))) goto S100; */
    ...
    if(_GE_(fabs(fc),fabs(fb))) goto S100;
    if(!(c != a)) goto S90;
    d = a;
    fd = fa;
S90:
...
/*     qrzero = fc >= 0.0e0 && fb <= 0.0e0 || fc < 0.0e0 && fb >= 0.0e0; */
    qrzero = _POSZ_(fc) && _NEGZ_(fb) || _NEG_(fc) && _POSZ_(fb);
    if(!qrzero) goto S250;
    *status = 0;
    goto S260;
S250:
