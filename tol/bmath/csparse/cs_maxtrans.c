...
static void cs_augment (int k, cs *A, int *jmatch, int *cheap, int *w,
	int *js, int *is, int *ps)
{
	...
	for (p = ps [head] ; p < Ap [j+1] ; p++)    /* dfs at node j */
	{
...
int *cs_maxtrans (cs *A)   /* returns jmatch [0..m-1]; imatch [0..n-1] */
{
	...
	*ps, *Ai, *Cp, *jmatch, *imatch ;
    ...
    C = (m2 < n2) ? cs_transpose (A,0) : A ;	    /* transpose if needed */
