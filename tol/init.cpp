...
//--------------------------------------------------------------------
{
  ...
  if ( tm0 && (tm1 - *tm0) < 1 ) {
    Error("SIGNAL: Interactive attention signal; on interactive systems this is usually generated by typing some break-in key at the terminal. (Exit application)");
    exit(sig);
  } else {
    Error("SIGNAL: Interactive attention signal; on interactive systems this is usually generated by typing some break-in key at the terminal. (Stop evaluation)");
    //Error_UnexpectedTolEnd();
    StopFlagOn();
    if ( !tm0 ) {
      tm0 = new time_t;
