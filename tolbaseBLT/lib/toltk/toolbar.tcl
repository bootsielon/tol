  ...
  if {$DefaultOption(ToolbarImageHandle)} {
    ...
    if {![string compare $orientation H]} {
      ...
      frame $toolbar.handle.l2 -relief $HandleRelief -bd 1 -width 2 \
        -background $DefaultOption(ToolbarHandleBackground)
      pack $toolbar.handle.l1 -fill y -side left -padx 2 -pady 2
      #Pedro pady 14 ... primer Boton siempre de ese tamaño
      pack $toolbar.handle.l2 -fill y -side left -pady 13
    } else {
...
#
#/////////////////////////////////////////////////////////////////////////////
proc ::toolbar::GetCommandList {type args} {
#
# PURPOSE: Crea la lista de comandos a seleccionar por el usuario al crear
#          un nuevo botón desde Tolbase
#/////////////////////////////////////////////////////////////////////////////
  variable widget
  variable data
  ...
  switch $type {
    "prj" {
      if {[llength $data(toolbar,prj,lstcmd)]} {
        set lst $data(toolbar,prj,lstcmd)
      }
    }
    "sadd" {
      if {[llength $data(toolbar,sadd,lstcmd)]} {
        set lst $data(toolbar,sadd,lstcmd)
      }
    }
    "tolbase" {
      if {[llength $data(toolbar,tolbase,lstcmd)]} {
        set lst $data(toolbar,tolbase,lstcmd)
      }
    }
  }
  
  puts "lista de valores -> $lst"
  return $lst
}

proc ::toolbar::HandleCallback {toolbar {event 1} { type none} } {
  variable ToolbarSpecifications
  
  #Utilizo el evento 40 en caso de que los 39 anteriores ya esten ocupados...
  #pedro
  #Toolbar Wizzard
  
  if {$event == 40} {
    # creo la lista de formularios a elegir
    set cmdlista [::toolbar::GetCommandList $type]
    ::newButtonDiag2::Show \
      -toolbar $toolbar \
      -cmdlst  $cmdlista
...
proc ::toolbar::ContextSensitiveMenu {toolbar} {
  ...
  set mbut [menu $menu.mbut -tearoff 0]


  #Primera Opcion
  if {$ToolbarSpecifications(HideToolbar:$toolbar)} {
    set butMen [$menu add cascade -label [::msgcat::mc "Nuevo Boton"] \
      -menu $mbut] 
  }
  
  #SUBMENU BOTONES
  $mbut add command -label [mc "Boton TolBase"] \
    -command [list ::toolbar::HandleCallback $toolbar 40 "tolbase"]
  
  $mbut add command -label [mc "Boton SADD"] \
    -command [list ::toolbar::HandleCallback $toolbar 40 "sadd"]
  
  $mbut add command -label [mc "Boton Proyecto"] \
    -command [list ::toolbar::HandleCallback $toolbar 40 "prj"]
  
    
  #Segunda Opcion
  set parent [winfo parent $toolbar]
   if {$ToolbarSpecifications(HideToolbar:$toolbar)} {
    $menu add command -label [::msgcat::mc "Nueva Toolbar"] -command \
        "::newToolbarDiag::Show \
          -toolbartype \"toolbar\" \
          -toolbarparent $parent"
  }
  if {$ToolbarSpecifications(HideToolbar:$toolbar)} {
    $menu add command -label [::msgcat::mc "Nuevo Frame de Toolbars"] -command \
        "::newToolbarDiag::Show \
          -toolbartype \"toolbarframe\""
  }
  if {$ToolbarSpecifications(FloatToolbar:$toolbar)} {
    $menu add command -label [::msgcat::mc Desatar] -command \
        "::toolbar::HandleCallback $toolbar -1"
  }
  #if {$ToolbarSpecifications(HideToolbar:$toolbar)} {
  #  $menu add command -label [::msgcat::mc Ocultar] -command \
  #      "::toolbar::HandleCallback $toolbar -2"
  #}
